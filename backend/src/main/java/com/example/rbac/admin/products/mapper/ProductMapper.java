...
import com.example.rbac.admin.products.model.ProductInfoSection;
...
public class ProductMapper {
    ...
    private List<Long> mapFrequentlyBoughtCategoryIds(Set<com.example.rbac.admin.categories.model.Category> categories) {
        if (categories == null || categories.isEmpty()) {
            return List.of();
        }
        return categories.stream()
                .filter(Objects::nonNull)
                .map(com.example.rbac.admin.categories.model.Category::getId)
                .filter(Objects::nonNull)
                .collect(Collectors.toList());
    }
