...
@Service
public class WedgeIconStorageService {
    ...
    public WedgeIconStorageService(
            @Value("${app.wedge.icon-storage-path:storage/catalog/wedges/icons}") String storagePath,
            @Value("${app.wedge.public-base-url:${app.brand.public-base-url:${APP_PUBLIC_BASE_URL:http://localhost:8080}}}")
        ...
        try {
            ...
            throw new IllegalStateException("Failed to initialize wedge icon storage directory", ex);
        }
    }
    ...
    public StoredIcon store(MultipartFile file) {
        ...
        if (contentType == null || !contentType.toLowerCase().startsWith("image/")) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Only image uploads are allowed for wedge icons");
        }
        ...
        try {
            ...
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to store wedge icon");
        }
    }
    ...
    public Resource load(String key) {
        ...
        throw new ApiException(HttpStatus.NOT_FOUND, "Wedge icon not found");
    }
    ...
    public String publicUrlForKey(String key) {
        ...
        return resolvePublicUrl("/api/v1/wedges/assets/" + key);
    }
